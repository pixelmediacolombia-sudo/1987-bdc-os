...
DROP POLICY IF EXISTS facts_tenant_isolation ON public.facts;
CREATE POLICY facts_tenant_isolation ON public.facts AS PERMISSIVE FOR ALL TO PUBLIC
...
CREATE POLICY objectives_tenant_isolation ON public.objectives AS PERMISSIVE FOR ALL TO PUBLIC
  USING (tenant_id = (NULLIF(current_setting('app.tenant_id', true), '')::uuid))
  WITH CHECK (tenant_id = (NULLIF(current_setting('app.tenant_id', true), '')::uuid));
`;
