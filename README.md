...
El Ticket 7 añade el motor determinista de siguiente acción en `src/modules/decisions`. `REPLY` y cualquier otra acción deben quedar dentro de `allowed_actions` antes de invocar un LLM. La política `koons_policy_v1` usa quiet hours de 21:00 a 08:00 y deja `downPayment.min` sin definir hasta que gerencia configure la banda aprobada; un down payment explícitamente igual a cero sigue requiriendo el disclaimer estructurado. El endpoint `GET /tests/policy-evaluation` requiere `X-Policy-Diagnostic-Token` y `POLICY_DIAGNOSTIC_TOKEN`; recibe `tenant_id`, `contact_id` y `payload` JSON URL-encoded, registra la decisión bajo RLS y no envía mensajes.
