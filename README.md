...
Sofía se encuentra preparada detrás de `SOFIA_ENABLED=false`. En esta fase el motor es determinista y de persistencia únicamente: toma el turno entrante consolidado, conserva los campos de financiación definidos en la especificación, recalcula el nivel A/B/C y guarda el estado con RLS por dealer. La ejecución requiere además `tenants.sofia_enabled = true`; el envío requiere también `SOFIA_ENABLED=true`, `QUALIFICATION_FLOW_ENABLED=true` y `tenants.qualification_flow_enabled=true`. Los tres flags por tenant (`sofia_enabled`, `qualification_flow_enabled`, `qualification_signal_enabled`) nacen en `false`, y se activan explícitamente uno por uno. `SOFIA_DEALER_NAME` solo personaliza las preguntas planificadas; las reglas financieras específicas del dealer deben sustituir los valores de referencia antes de una activación.
