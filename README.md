...
La aplicación ejecuta migraciones idempotentes al iniciar y garantiza que `public.integrations` exista antes de escuchar peticiones. También está disponible `npm run db:migrate` para ejecutarlas de forma explícita; incluye las columnas de flags por tenant, las columnas de atribución, `capi_events`, la bitácora del tag de Ticket 8.5, la configuración aislada de dealer (`dealer_name`, `financial_policy`, `facebook_page_id`) y `conversation_ai_shadow_runs` con RLS.
...
El flujo de calificación del Ticket 8 se compone mediante `QUALIFICATION_FLOW_ENABLED`, desactivado por defecto. Cuando está activo, el orquestador exige `QualificationFlowService`, que encadena el ledger/policy evaluator, el registro PostgreSQL, el veto de repetición semántica y el proveedor outbound de GHL antes de permitir un envío. La respuesta efectiva requiere además `tenants.qualification_flow_enabled = true` para el tenant del mensaje.
...
El worker permanece apagado con `QUALIFICATION_SIGNAL_ENABLED=false`. Al habilitarlo, el tag `qualification_completed` se procesa de forma independiente, pero solo para tenants con `qualification_signal_enabled = true`. Meta procesa un tenant si tiene el par de variables de entorno que corresponde a su nombre; como compatibilidad, también acepta `meta_capi_enabled=true` con dataset y token cifrado configurados en `tenants`. `meta_test_event_code` se ignora en producción y provoca fallo de contrato si aparece en una entrega de producción. `QUALIFICATION_SIGNAL_POLL_MS` controla el intervalo del worker (5 segundos por defecto). No se envían mensajes de conversación como parte de este ticket.
...
Sofía se encuentra preparada detrás de `SOFIA_ENABLED=false`. En esta fase el motor es determinista y de persistencia únicamente: toma el turno entrante consolidado, conserva los campos de financiación definidos en la especificación, recalcula el nivel A/B/C y guarda el estado con RLS por dealer. La ejecución requiere además `tenants.sofia_enabled = true`; el envío requiere también `SOFIA_ENABLED=true`, `QUALIFICATION_FLOW_ENABLED=true` y `tenants.qualification_flow_enabled=true`. Los flags por tenant nacen en `false`, y se activan explícitamente uno por uno. `SOFIA_DEALER_NAME` solo personaliza las preguntas planificadas; las reglas financieras específicas del dealer deben sustituir los valores de referencia antes de una activación.

La capa híbrida usa `CONVERSATIONAL_AI_ENABLED=false` por defecto. Cuando se autorice, el adapter OpenAI-compatible ejecuta extracción JSON y redacción por separado, con modelos configurables, prompts versionados y fallback determinista. `conversational_ai_shadow_enabled` guarda la respuesta del modelo junto a la regla en `conversation_ai_shadow_runs`; `conversational_ai_send_enabled` permanece apagado hasta revisar dos semanas de sombra. La API key no se necesita para build ni pruebas locales: las pruebas usan un fake local con el mismo contrato.

La policy `country_club_cars_v8` se activa por tenant mediante la migración idempotente `ensureCountryClubPolicy`; solo actualiza el `ghl_location_id` explícitamente identificado. El motor de Sofía es de código: extrae hechos, valida rangos/coherencia y redacta respuestas con reglas deterministas. La matriz visible del PDF está cubierta por 60 pruebas en `test:country-club:matrix`; la prueba `test:country-club:db10` ejecuta 10 conversaciones contra PostgreSQL local y comprueba respuestas y estado persistido.
