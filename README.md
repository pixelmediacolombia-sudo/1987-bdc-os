...
Los datasets y tokens de Meta pueden configurarse directamente mediante `META_CAPI_DATASET_COUNTRY_CLUB` / `META_CAPI_TOKEN_COUNTRY_CLUB`, `META_CAPI_DATASET_KOONS_CULPEPER` / `META_CAPI_TOKEN_KOONS_CULPEPER` y `META_CAPI_DATASET_EASTERNS_ES` / `META_CAPI_TOKEN_EASTERNS_ES`. El token solo vive en memoria durante el envío: no se incluye en `payload_sent`, ni en logs, ni en el repositorio. El worker asocia cada par al tenant cuyo registro contiene el nombre correspondiente; los tenants no identificados no reciben el evento.
